package br.com.segsat.restwhitspringbootandjava.security.jwt;
...
import br.com.segsat.restwhitspringbootandjava.exceptions.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
    ...
    public TokenVO refreshToken(String refresToken) {
        if (refresToken.contains("Bearer ")){
            refresToken = refresToken.substring("Bearer ".length());
        }
        JWTVerifier verifier = JWT.require(algorithm).build();
        DecodedJWT decodedJWT = verifier.verify(refresToken);
    ...
    private String getRefreshToken(String username, List<String> roles, Date now) {
        Date validityRefresh = new Date(now.getTime() + (validityInMilliseconds * 3));
        return JWT.create()
                .withClaim("roles", roles)
                .withIssuedAt(now)
